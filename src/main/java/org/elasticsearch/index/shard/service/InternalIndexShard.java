...
public class InternalIndexShard extends AbstractIndexShardComponent implements IndexShard {
    ...
    public ParsedDocument create(Engine.Create create) throws ElasticsearchException {
        ...
        // we can only do the auto generated id optimization if the shard is in
        // fully started mode (and not, for example, in recovery / post_recovery)
        if (create.autoGeneratedId()) {
            if (state != IndexShardState.STARTED) {
                create.autoGeneratedId(false);
            }
        }
